...
std::pair<int,int> makeAllGroups(const std::map<double, std::vector<std::pair<int, int>>> dist, const int &n) {
    disJointSet ds(n);
    int amt{};
    bool end = false;
    while (true) {
        for (auto& curr_dist : dist) {
            for (auto& coord : curr_dist.second) {
                ...
                if (ds.getTopMap() == 1000) return { coord.first, coord.second };
            }
        }
    }
}
...
ll runOnce() {
    ...
    std::pair<int,int> last = makeAllGroups(distances, n);
    ll res = 1LL * coords[last.first][0] * 1LL * coords[last.second][0];
    return res;
}
