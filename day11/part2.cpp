...
ll allPaths(int curr, int end,
    bool seen_dac, bool seen_fft,
    int dac, int fft,
    const std::unordered_map<int, std::vector<int>>& adj,
    std::unordered_map<ll, ll>& mem)
{
    ll key = ((ll)curr << 2) | (seen_dac << 1) | seen_fft;
    ...
    for (int nxt : it->second) {
        bool nd = seen_dac || (nxt == dac && nxt != end);
        bool nf = seen_fft || (nxt == fft && nxt != end);
        ...
        if (nxt == end) {
            ...
            tot += allPaths(nxt, end, nd, nf, dac, fft, adj, mem);
        }
    }
...
ll getDataAndRun() {
    ...
    infile.close();
    int start_search = string_map["svr"];
    int end_search = string_map["out"];
	int dac = string_map["dac"];    
	int fft = string_map["fft"];
    /*for (auto& el : adj_map) {
    ...
    for (int i = 1; i <= RUNS; i++) {
        ...
        std::unordered_map<ll, ll> mem;
        ll result = allPaths(start_search, end_search, false, false, dac, fft, adj_map, mem);
